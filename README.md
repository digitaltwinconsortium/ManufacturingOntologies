...
1. From an **Windows command prompt** on a Windows 11 PC, run `wsl`. This will start the Windows Subsystem for Linux. If this is not yet installed, you can install it by running `wsl --install`. Then follow the instructions [here](https://learn.microsoft.com/en-us/azure/iot-operations/manage-devices-assets/howto-configure-opcua-authentication-options#configure-opc-ua-transport-authentication) and [here](https://learn.microsoft.com/en-us/azure/iot-operations/manage-devices-assets/howto-configure-opcua-authentication-options#create-a-self-signed-certificate-for-transport-authorization) to create an OPC UA certificate and private key for Azure IoT Operations. The step `Configure the secret provider class aio-opc-ua-broker-client-certificate custom resource` will be completed later. 
