...
1. Once deployment completes and all Kubernetes pods are up and running, log in to https://iotoperations.azure.com, pick the right Azure directory (top right hand corner) and start creating assets from the production line simulation. As mentioned above, the solution comes with 2 prouction lines (Munich and Seattle) consisting of 3 stations each (assembly, test and packaging).
1.1. For the asset endpoints, enter opc.tcp://assembly.munich in the OPC UA Broker URL field for the assembly station of the Munich production line, etc.
1.1. For the asset tags, enter ns=1;NodeID, where NodeID is the OPC UA node ID as described above [here](https://github.com/digitaltwinconsortium/ManufacturingOntologies?tab=readme-ov-file#opc-ua-node-ids-of-station-opc-ua-server). I.e., for the manufactured product serial number enter ns=1;i=379.
